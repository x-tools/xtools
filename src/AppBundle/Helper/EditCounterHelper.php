...
class EditCounterHelper
{
     ...
     * @throws Exception
     */
    public function getRevisionCounts($usernameOrIp)
    {
        ...
        $resultQuery = $this->replicas->prepare("
            (SELECT 'deleted' as source, COUNT(*) AS value FROM $archiveTable
                WHERE ar_user_text = :username)
            UNION
            (SELECT 'live' as source, COUNT(*) AS value FROM $revisionTable
        ...
        if (count($results) < 8) {
            throw new Exception("Unable to get all revision counts for '$usernameOrIp'");
        }
